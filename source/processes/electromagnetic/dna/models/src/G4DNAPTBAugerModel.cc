...
G4int G4DNAPTBAugerModel::DetermineIonisedAtom(G4int atomId, const G4String& materialName, G4double bindingEnergy)
{
    ...
    {
        if(bindingEnergy==307.52){  // todo - for PU its not exactly that, so it will not trigger?? MPietrzak
            atomId=1; //"carbon";
        }
        else if(bindingEnergy==423.44){  // todo - same story here. MPietrzak
            atomId=4; //"nitrogen";
        }
    }
    else if(materialName=="TMP"|| materialName=="backbone_TMP"){
        if(bindingEnergy==209.59 || bindingEnergy==152.4)  // todo - shouldn't it be 152.42 instead of 152.4?? MPietrzak
            atomId=3; //"carbonTMP";
    }
